#!/usr/bin/env bash
# Invoice print/preview template integration test orchestrator
# (INV-UI01..14), followed by the FULL existing Phase 0-5 + UI + Print
# regression suite (INV-UI15) — this preview-only Invoice template must
...
# STORE A/B), then runs PhaseInvoiceUiTest.php against a live `php -S`
# server, and finally re-runs every prior phase's own orchestrator script
# in sequence (via run-print-preview.sh, which itself delegates onward).
#
# Usage: bash api/tests/run-invoice-ui-preview.sh
set -uo pipefail
...
DB_NAME="amor_factory_invoice_test"
...
ADMIN_PASSWORD="$TEST_ADMIN_PASS" php "$API_ROOT/bin/create_admin.php" invoice_staging_admin "Invoice Staging Admin" || { echo "create_admin.php FAILED"; exit 1; }
...
echo "--- 8/9: starting php -S dev server on :$PHP_PORT (with the shared _ui_router.php, so /api/app/ui/assets/*.css|png serve correctly) and running PhaseInvoiceUiTest.php (INV-UI01..14) ---"
...
TEST_ADMIN_USER="invoice_staging_admin" \
TEST_ADMIN_PASS="$TEST_ADMIN_PASS" \
TEST_DB_SOCKET="$SOCK" \
TEST_DB_NAME="$DB_NAME" \
TEST_RUNTIME_USER="inv_runtime_user" \
TEST_RUNTIME_PASS="$RUNTIME_USER_PASS" \
php "$API_ROOT/tests/PhaseInvoiceUiTest.php"
INV_RESULT=$?

kill "$PHP_PID" 2>/dev/null || true
PHP_PID=""

echo "--- 9/9: INV-UI15 — full existing Phase 0-5 + UI + Print regression ---"
# run-print-preview.sh's own final step already re-runs run-ui-preview.sh,
# which in turn re-runs every prior phase's orchestrator (run.sh through
# run-phase5-do-shipment.sh) in sequence, so calling it alone covers the
# full regression without running each script twice.
REGRESSION_RESULT=0
echo "  -> run-print-preview.sh (covers PRINT-01..13 plus the full Phase 0-5 + UI regression)"
bash "$API_ROOT/tests/run-print-preview.sh" > "$WORKDIR/run-print-preview.sh.log" 2>&1
RC=$?
if [ $RC -ne 0 ]; then
  echo "     FAILED — last 40 lines of run-print-preview.sh's own output:"
  tail -40 "$WORKDIR/run-print-preview.sh.log"
  REGRESSION_RESULT=1
else
  tail -5 "$WORKDIR/run-print-preview.sh.log"
fi

echo ""
if [ $INV_RESULT -eq 0 ]; then echo "INV-UI01..INV-UI14: PASSED"; else echo "INV-UI01..INV-UI14: FAILED"; fi
if [ $REGRESSION_RESULT -eq 0 ]; then echo "INV-UI15 (full Phase 0-5 + UI + Print regression): PASSED"; else echo "INV-UI15 (full Phase 0-5 + UI + Print regression): FAILED"; fi
