#!/usr/bin/env bash
# Phase 3 fast-track Production/SPK actual module integration test
# orchestrator (P3-01..P3-24).
#
...
# post-Phase-2 state (migrations 0001+0002+0003 applied, seeded, ADMIN
# created, 8 divisions + all 472 katalog products + the BAKERY CIKOLE alias
# group imported via Phase1Importer — exactly what a real deployment looks
# like right after Phase 2 PO review, before this Phase 3 patch's migration
# 0004 has been touched), then runs Phase3ProductionTest.php (P3-01..P3-24)
# against a live `php -S` server.
#
# Usage: bash api/tests/run-phase3-production.sh
set -uo pipefail
...
DB_NAME="amor_factory_phase3production_test"
TEST_ADMIN_PASS="StagingAdmin#$(date +%s)P3"
MIGRATION_USER_PASS="MigrationUserPassP3_123"
RUNTIME_USER_PASS="RuntimeUserPassP3_123"
PHP_PORT=8098
PHP_PID=""
MIGRATION_0004="$API_ROOT/app/migrations/0004_production_phase3.php"
...
cleanup() {
  ...
  if [ -f "$MIGRATION_0004_PARKED" ] && [ ! -f "$MIGRATION_0004" ]; then
...
CREATE USER 'p3_migration_user'@'localhost' IDENTIFIED BY '$MIGRATION_USER_PASS';
GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'p3_migration_user'@'localhost';
CREATE USER 'p3_runtime_user'@'localhost' IDENTIFIED BY '$RUNTIME_USER_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON ${DB_NAME}.* TO 'p3_runtime_user'@'localhost';
FLUSH PRIVILEGES;
"

echo "--- 4/10: parking migration 0004 so bootstrap only applies 0001-0003 (0004 must be genuinely PENDING) ---"
...
return [
    ...
    'DB_USER' => 'p3_migration_user',
    'DB_PASS' => '$MIGRATION_USER_PASS',
    'SESSION_SECURE' => false,
];
PHPCONFIG

echo "--- 6/10: migrate (0001+0002+0003) + seed + create ADMIN ---"
php "$API_ROOT/bin/migrate.php" --yes || { echo "migrate.php FAILED"; exit 1; }
php "$API_ROOT/bin/seed.php" || { echo "seed.php FAILED"; exit 1; }
ADMIN_PASSWORD="$TEST_ADMIN_PASS" php "$API_ROOT/bin/create_admin.php" p3_staging_admin "P3 Staging Admin" || { echo "create_admin.php FAILED"; exit 1; }
...
echo "--- 8/10: restoring migration 0004 (now genuinely pending) ---"
mv "$MIGRATION_0004_PARKED" "$MIGRATION_0004"
...
return [
    ...
    'DB_NAME' => '$DB_NAME',
    'EXPECTED_DB_NAME' => '$DB_NAME',
    'DB_USER' => 'p3_runtime_user',
    'DB_PASS' => '$RUNTIME_USER_PASS',
    'MIGRATION_DB_HOST' => 'unused-socket-mode',
    'MIGRATION_DB_SOCKET' => '$SOCK',
    'MIGRATION_DB_NAME' => '$DB_NAME',
    'MIGRATION_DB_USER' => 'p3_migration_user',
    'MIGRATION_DB_PASS' => '$MIGRATION_USER_PASS',
    'SESSION_SECURE' => false,
];
PHPCONFIG

echo "--- 10/10: starting php -S dev server on :$PHP_PORT and running Phase3ProductionTest.php (P3-01..P3-24) ---"
...
TEST_ADMIN_USER="p3_staging_admin" \
TEST_ADMIN_PASS="$TEST_ADMIN_PASS" \
TEST_DB_SOCKET="$SOCK" \
TEST_DB_NAME="$DB_NAME" \
TEST_RUNTIME_USER="p3_runtime_user" \
TEST_RUNTIME_PASS="$RUNTIME_USER_PASS" \
php "$API_ROOT/tests/Phase3ProductionTest.php"
RESULT=$?
