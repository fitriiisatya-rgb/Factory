...
/**
 * Bulk print page for the Phase 5 Draft DO wizard — one DO per printed
 * page (page-break-after between each), for a whole date+factory at once.
 * Read-only. Same DRAFT/PREPRINT watermark rule as print.php, applied
...
try {
    ...
    http_response_code(200);
    echo 'Gagal memuat daftar DO: ' . htmlspecialchars($e->getMessage());
    exit;
}
...
<title>Print Bulk DO — <?= esc($tanggal) ?></title>
<style>
body{font-family:Arial,sans-serif;color:#111;}
.doc-page{max-width:800px;margin:1.5rem auto;padding:0 1rem;position:relative;page-break-after:always;}
...
.watermark{position:absolute;top:40%;left:0;right:0;text-align:center;font-size:2.6rem;font-weight:bold;color:rgba(200,0,0,.18);
...
<body>
<div class="no-print"><button onclick="window.print()">Cetak Semua</button> <a href="index.php?tanggal=<?= urlencode($tanggal) ?>&factoryId=<?= $factoryId ?>">&larr; Kembali</a>
  &middot; <?= count($docs) ?> DO</div>
...
<?php foreach ($docs as $do):
  $watermark = null;
  if ($do['status'] === 'draft') { $watermark = 'DRAFT — BELUM DICETAK RESMI'; }
  elseif ($do['status'] === 'preprinted') { $watermark = 'PREPRINT — BELUM DIKIRIM'; }
