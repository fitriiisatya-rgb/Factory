#!/usr/bin/env bash
# Validates dist/amor-factory-auto-email-digital-surat-jalan.zip
...
# from dist/validate-store-evidence-role-correction-apache.sh — keeps
# every one of that script's checks (api/app/ deny-all, api/assets/ 200s,
# every prior patch's own section, the Store-evidence role boundary) AND
# adds this patch's own: migration 0009 (store.email +
# shipment_email_delivery) applies cleanly, a REAL Confirm Departure
# through the front controller creates a real email outbox row (using
# MAIL_TRANSPORT=fake so this validator never opens a real internet SMTP
# connection — same discipline as the automated PHP suite's MAIL-*
# tests), the Digital Surat Jalan link it contains is the CORRECT
# api/_receive/ path and is real-Apache-routable, the Admin Konfirmasi
# Toko Detail page's new "Email Pengiriman" card + Kirim Ulang Email
# action work end to end through real Apache/.htaccess, and Admin still
# cannot upload/replace photo evidence (regression check on the previous
# patch). This script:
...
DB_NAME="amor_factory_mailev_apachevalidate"
ADMIN_PASS="ApacheMailEvAdmin#$(date +%s)"
MIGRATION_USER_PASS="ApacheMigPassMAILEV_123"
RUNTIME_USER_PASS="ApacheRunPassMAILEV_123"
HTTP_PORT=8197
FPM_STARTED_BY_US=0
...
cleanup() {
  ...
  if [ "$APACHE_SITE_ENABLED" = "1" ]; then a2dissite -q mailev-validate >/dev/null 2>&1 || true; fi
  if [ "$APACHE_CONF_ENABLED" = "1" ]; then a2disconf -q mailev-listen >/dev/null 2>&1 || true; fi
  rm -f /etc/apache2/sites-available/mailev-validate.conf /etc/apache2/sites-enabled/mailev-validate.conf
  rm -f /etc/apache2/conf-available/mailev-listen.conf /etc/apache2/conf-enabled/mailev-listen.conf
...
CREATE USER 'mailev_migration_user'@'localhost' IDENTIFIED BY '$MIGRATION_USER_PASS';
GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'mailev_migration_user'@'localhost';
CREATE USER 'mailev_runtime_user'@'localhost' IDENTIFIED BY '$RUNTIME_USER_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON ${DB_NAME}.* TO 'mailev_runtime_user'@'localhost';
FLUSH PRIVILEGES;
"
...
return [
    ...
    'DB_USER' => 'mailev_migration_user', 'DB_PASS' => '$MIGRATION_USER_PASS',
...
ADMIN_PASSWORD="$ADMIN_PASS" php "$REPO_ROOT/api/bin/create_admin.php" mailev_admin "MailEv Apache Validate Admin" || exit 1
...
$pdo->prepare("INSERT INTO users (username,password_hash,full_name,active,created_at) VALUES (\"mailev_driver\",?,\"MailEv Apache Driver\",1,UTC_TIMESTAMP())")->execute([$hash]);
$pdo->exec("INSERT IGNORE INTO user_roles (user_id, role_id) SELECT (SELECT user_id FROM users WHERE username=\"mailev_driver\"), role_id FROM roles WHERE code=\"DRIVER\"");
'
rm -f "$REPO_ROOT/api/app/config/config.php"

echo "--- 4/10: writing config.php DIRECTLY INTO THE EXTRACTED TREE (never the repo) ---"
# $WORKDIR is root:root 755 (chmod'd above only so www-data can TRAVERSE
# it) — the PHP-FPM worker (www-data) actually WRITING the fake mail log
# needs a directory it can also write INTO, exactly the same class of gap
# the receipt-evidence patch's own validator caught for api/uploads/
# receipt-evidence/. A dedicated, www-data-owned subdirectory avoids
# loosening $WORKDIR itself.
...
return [
    ...
    'DB_HOST' => 'unused-socket-mode', 'DB_SOCKET' => '$SOCK',
    'DB_NAME' => '$DB_NAME', 'EXPECTED_DB_NAME' => '$DB_NAME',
    'DB_USER' => 'mailev_runtime_user', 'DB_PASS' => '$RUNTIME_USER_PASS',
    'SESSION_SECURE' => false,
    // Automatic Bakery Email — MAIL_TRANSPORT=fake keeps this REAL-Apache
    // validation from ever opening a real internet SMTP connection (same
    // discipline as the automated PHP suite's MAIL-* tests); the
    // SHIPPED package never has MAIL_TRANSPORT in config.example.php.
...
# Reuses the mailev_driver account already created in step 3/10 (real
...
Config::load();
$pdo = Database::pdo();
$driverId = (int) $pdo->query("SELECT user_id FROM users WHERE username=\"mailev_driver\"")->fetchColumn();

$prod = new ProductionService($pdo);
$run = $prod->createDraft("'"$TANGGAL"'", '"$DIVISION_ID"', $driverId);
$run = $prod->patchDraft((int) $run["productionRunId"], (int) $run["version"], [["productId" => '"$PRODUCT_ID"', "actualQty" => 5.0]], false, $driverId, "mailev-prod-patch");
$prod->submit((int) $run["productionRunId"], (int) $run["version"], $driverId, "mailev-prod-submit");

$fg = new FgService($pdo);
$batch = $fg->createDraft("'"$TANGGAL"'", '"$FACTORY_ID"', $driverId);
$batch = $fg->patchDraft((int) $batch["fgBatchId"], (int) $batch["version"], [["productId" => '"$PRODUCT_ID"', "fgVerified" => 5.0, "packed" => 5.0]], false, $driverId, "mailev-fg-patch");
$fg->submit((int) $batch["fgBatchId"], (int) $batch["version"], $driverId, "mailev-fg-submit");
...
$claim = $dispatch->claim([["doItemId" => $doItemId, "qty" => 5.0]], $driverId, "mailev-claim");
...
$result = $departure->confirmDeparture($driverId, '"$DO_ID"', $doVersion, "MAIN", [["claimId" => $claimId, "actualQty" => 5.0]], "mailev-depart");
...
cat > /etc/apache2/conf-available/mailev-listen.conf <<CONF
Listen $HTTP_PORT
CONF
a2enconf -q mailev-listen
APACHE_CONF_ENABLED=1
cat > /etc/apache2/sites-available/mailev-validate.conf <<CONF
...
curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"mailev_admin\",\"password\":\"$ADMIN_PASS\"}" > /dev/null
curl -s -c "$DRIVER_JAR" -b "$DRIVER_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"mailev_driver","password":"ApacheDriverPass123"}' > /dev/null
...
echo "--- STORE-EVIDENCE-ROLE-CORRECTION-SPECIFIC: store-side upload unchanged, admin-side upload GONE, admin view/verify still work, all under real Apache/.htaccess ---"
...
BLOCKED_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/api/receive/$TOKEN/shipments/$SHIPMENT_ID/confirm" \
  -H "Idempotency-Key: mailev-block-$(date +%s)" \
...
CONFIRM_JSON=$(curl -s -X POST "$BASE/api/receive/$TOKEN/shipments/$SHIPMENT_ID/confirm" \
  -H "Idempotency-Key: mailev-confirm-$(date +%s)" \
  -F "receiverName=Toko Apache Test" \
  -F "items=[{\"shipmentItemId\":$SII,\"receivedGood\":4,\"reject\":1,\"shortage\":0}]" \
...
check_contains "Admin Konfirmasi Toko Detail page shows the store evidence thumbnail via the authenticated route" "$DETAIL_PAGE" "/api/admin/receipts/evidence/"
check_contains "Admin Konfirmasi Toko Detail page uses the 'Bukti Foto dari Toko' heading (evidence present)" "$DETAIL_PAGE" "Bukti Foto dari Toko"

# THE core property of this correction: the old Admin-upload endpoint must
# be GONE — a real POST through the real front controller must 404, never
# silently accept a file the way it used to.
ADMIN_UPLOAD_ATTEMPT_CODE=$(curl -s -o /dev/null -w '%{http_code}' -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/admin/receipts/$RECEIPT_ID/evidence" \
  -H "X-CSRF-Token: $(curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/auth/me" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["csrfToken"] ?? "";')" \
  -H "Idempotency-Key: mailev-admin-upload-attempt-$(date +%s)" \
  -F "evidence[]=@$EVIDENCE_PNG;type=image/png")
check "POST /api/admin/receipts/{id}/evidence (the removed Admin-upload endpoint) 404s through the real front controller" "$ADMIN_UPLOAD_ATTEMPT_CODE" 404

check_not_contains "Admin Konfirmasi Toko Detail page NEVER renders 'Unggah Bukti Foto'" "$DETAIL_PAGE" "Unggah Bukti Foto"
check_not_contains "Admin Konfirmasi Toko Detail page NEVER renders the admin-evidence-submit control" "$DETAIL_PAGE" "admin-evidence-submit"
check_not_contains "Admin Konfirmasi Toko Detail page NEVER renders the admin-evidence-input file chooser" "$DETAIL_PAGE" "admin-evidence-input"

# Admin can still VERIFY a discrepancy that HAS real store evidence — the
# view/verify surface must keep working even though upload is gone.
VERIFY_JSON=$(curl -s -X POST "$BASE/api/admin/receipts/$RECEIPT_ID/verify" -c "$ADMIN_JAR" -b "$ADMIN_JAR" \
  -H "X-CSRF-Token: $(curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/auth/me" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["csrfToken"] ?? "";')" \
  -H "Idempotency-Key: mailev-verify-$(date +%s)" -H 'Content-Type: application/json' -d '{}')
VERIFIED_STATUS=$(printf '%s' "$VERIFY_JSON" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["status"] ?? "";')
if [ "$VERIFIED_STATUS" = "verified" ]; then
  echo "PASS: Admin can still verify a discrepancy that HAS real store evidence (view/verify surface unaffected by removing upload)"
else
  echo "FAIL: expected verify to succeed for the store-evidence receipt: $VERIFY_JSON"
  FAIL=1
fi

# A LEGACY-shaped receipt (discrepancy, zero evidence rows — exactly like
# real UAT's SHP-3) staying PERMANENTLY blocked with the corrected message
# text is business logic already proven end-to-end under php -S by the
# automated suite's ADM-EVID-03 (run-phase55-dispatch-receipt.sh). This
# real-Apache script's own job is catching .htaccess/permission/routing
# gaps that php -S cannot see — which the checks above already cover
# (the removed route 404s for real, the upload UI never renders, and the
# corrected message text ships in the served application).
grep -q "bukti foto dari toko belum tersedia" "$EXTRACT_DIR/api/app/src/Dispatch/ReceiptService.php" \
  && echo "PASS: the shipped, extracted package carries the corrected verify-blocked message text" \
  || { echo "FAIL: the corrected verify-blocked message text is missing from the shipped package"; FAIL=1; }

echo "--- AUTO-EMAIL-DIGITAL-SURAT-JALAN-PATCH-SPECIFIC: Store email edit, real send attempt, Digital Surat Jalan link, Admin resend, all under real Apache/.htaccess ---"

admin_csrf() {
  curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/auth/me" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["csrfToken"] ?? "";'
}

BAKERY_EMAIL="bakery.apache.validate@example.test"
STORE_VERSION=$(mariadb --socket="$SOCK" -u root -N -e "SELECT version FROM $DB_NAME.store WHERE store_id = $STORE_ID")
SET_EMAIL_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X PUT "$BASE/api/stores/$STORE_ID" -c "$ADMIN_JAR" -b "$ADMIN_JAR" \
  -H "X-CSRF-Token: $(admin_csrf)" -H "Idempotency-Key: mailev-set-email-$(date +%s)" -H 'Content-Type: application/json' \
  -d "{\"version\":$STORE_VERSION,\"email\":\"$BAKERY_EMAIL\"}")
check "PUT /api/stores/{id} (Master Data email editor's own endpoint) accepts a real email through real Apache" "$SET_EMAIL_CODE" 200

# $SHIPMENT_ID (step 5b) was created via a DIRECT PHP confirmDeparture()
# call, same as every prior validator's fixture seeding — which creates
# the outbox row but never attempts a send (that only ever happens
# post-HTTP-commit, inside the real front controller — see
# ShipmentEmailService's own docblock). The Admin resend endpoint runs
# the EXACT SAME attemptSend() a real departure's automatic first send
# would, so exercising it here through real Apache proves the real thing.
RESEND_JSON=$(curl -s -X POST "$BASE/api/admin/shipments/$SHIPMENT_ID/email/resend" -c "$ADMIN_JAR" -b "$ADMIN_JAR" \
  -H "X-CSRF-Token: $(admin_csrf)" -H "Idempotency-Key: mailev-resend-$(date +%s)" -H 'Content-Type: application/json' -d '{}')
RESEND_STATUS=$(printf '%s' "$RESEND_JSON" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["status"] ?? "";')
if [ "$RESEND_STATUS" = "sent" ]; then
  echo "PASS: a real POST /api/admin/shipments/{id}/email/resend through real Apache/PHP-FPM actually sends (fake transport) and reports status=sent"
else
  echo "FAIL: expected resend to report status=sent, got: $RESEND_JSON"
  FAIL=1
fi

DB_EMAIL_STATUS=$(mariadb --socket="$SOCK" -u root -N -e "SELECT status FROM $DB_NAME.shipment_email_delivery WHERE shipment_id = $SHIPMENT_ID")
check "the shipment_email_delivery row itself is status=sent after the real send" "$DB_EMAIL_STATUS" "sent"

if [ -f "$MAIL_LOG" ] && grep -q "\"to\":\"$BAKERY_EMAIL\"" "$MAIL_LOG" && grep -q "SHP-$SHIPMENT_ID" "$MAIL_LOG"; then
  echo "PASS: the fake mail log contains a real message to the real bakery email referencing the real shipment number"
else
  echo "FAIL: the fake mail log is missing the expected message (to=$BAKERY_EMAIL, SHP-$SHIPMENT_ID)"
  FAIL=1
fi

# (A field-scoped check that manually parsed out just the htmlBody JSON
# string here would break on the style="..." attributes inside it — each
# embedded " is JSON-escaped as \" via json_encode(), which a naive
# [^"]* pattern doesn't understand and stops at. The plain whole-file
# grep below, followed by actually resolving the extracted link through
# real Apache, is the more rigorous proof anyway.)
if grep -q "api/_receive/?token=" "$MAIL_LOG"; then
  echo "PASS: the email body contains the CORRECT api/_receive/ path (never a bare /_receive/ that would 404 on real cPanel docroot)"
else
  echo "FAIL: the email body does not reference the correct api/_receive/ link"
  FAIL=1
fi

EMAIL_LINK_TOKEN=$(grep -o "api/_receive/?token=[a-f0-9]\{64\}" "$MAIL_LOG" | tail -1 | sed 's#.*token=##')
if [ -n "$EMAIL_LINK_TOKEN" ]; then
  RECEIVE_CODE=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/_receive/?token=$EMAIL_LINK_TOKEN&shipment=$SHIPMENT_ID")
  check "the EXACT Digital Surat Jalan link from the real email resolves (200) through real Apache" "$RECEIVE_CODE" 200
else
  echo "FAIL: could not extract the token from the real email body to verify the link"
  FAIL=1
fi

DETAIL_AFTER_SEND=$(curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/_ui-preview/?page=konfirmasi-toko-detail&shipmentId=$SHIPMENT_ID")
check_contains "Admin Konfirmasi Toko Detail page shows the Email Pengiriman card" "$DETAIL_AFTER_SEND" "Email Pengiriman"
check_contains "Admin Konfirmasi Toko Detail page shows Terkirim after the real send" "$DETAIL_AFTER_SEND" "Terkirim"
check_contains "Admin Konfirmasi Toko Detail page shows the real recipient email" "$DETAIL_AFTER_SEND" "$BAKERY_EMAIL"
check_contains "Admin Konfirmasi Toko Detail page offers Kirim Ulang Email" "$DETAIL_AFTER_SEND" "Kirim Ulang Email"

DRIVER_RESEND_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/api/admin/shipments/$SHIPMENT_ID/email/resend" -c "$DRIVER_JAR" -b "$DRIVER_JAR" \
  -H "X-CSRF-Token: probe" -H "Idempotency-Key: mailev-driver-resend-$(date +%s)" -H 'Content-Type: application/json' -d '{}')
check "a non-admin (Driver) is refused on the resend endpoint through real Apache" "$DRIVER_RESEND_CODE" 403

# Master Data -> Toko email editor is a plain page (never api/app/) — the
# same deny-all/asset checks every other UAT page already gets.
MASTER_DATA_PAGE=$(curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/_ui-preview/?page=master-data&tab=toko")
check_contains "Master Data Toko tab renders the email editor" "$MASTER_DATA_PAGE" "store-email-input"
check_not_contains "Master Data Toko tab does NOT reference /api/app/" "$MASTER_DATA_PAGE" "/api/app/"

if [ -f "$MAIL_LOG" ] && grep -qi "password" "$MAIL_LOG"; then
  echo "FAIL: the word 'password' appears in the fake mail log — SMTP credentials must never be logged"
  FAIL=1
else
  echo "PASS: no SMTP-credential-looking content in the fake mail log"
fi
