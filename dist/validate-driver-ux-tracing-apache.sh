#!/usr/bin/env bash
# Validates dist/amor-factory-driver-ux-tracing-patch.zip against
...
# dist/validate-phase55-apache-assets.sh for the Driver Portal UX +
# Shipment Tracing patch specifically. This script:
...
DB_NAME="amor_factory_duxt_apachevalidate"
ADMIN_PASS="ApacheDuxtAdmin#$(date +%s)"
MIGRATION_USER_PASS="ApacheMigPassDUXT_123"
RUNTIME_USER_PASS="ApacheRunPassDUXT_123"
HTTP_PORT=8198
FPM_STARTED_BY_US=0
...
cleanup() {
  ...
  if [ "$APACHE_SITE_ENABLED" = "1" ]; then a2dissite -q duxt-validate >/dev/null 2>&1 || true; fi
  if [ "$APACHE_CONF_ENABLED" = "1" ]; then a2disconf -q duxt-listen >/dev/null 2>&1 || true; fi
  rm -f /etc/apache2/sites-available/duxt-validate.conf /etc/apache2/sites-enabled/duxt-validate.conf
  rm -f /etc/apache2/conf-available/duxt-listen.conf /etc/apache2/conf-enabled/duxt-listen.conf
...
CREATE USER 'duxtav_migration_user'@'localhost' IDENTIFIED BY '$MIGRATION_USER_PASS';
GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'duxtav_migration_user'@'localhost';
CREATE USER 'duxtav_runtime_user'@'localhost' IDENTIFIED BY '$RUNTIME_USER_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON ${DB_NAME}.* TO 'duxtav_runtime_user'@'localhost';
FLUSH PRIVILEGES;
"
...
return [
    ...
    'DB_USER' => 'duxtav_migration_user', 'DB_PASS' => '$MIGRATION_USER_PASS',
...
ADMIN_PASSWORD="$ADMIN_PASS" php "$REPO_ROOT/api/bin/create_admin.php" duxtav_admin "DUXT Apache Validate Admin" || exit 1
...
$pdo->prepare("INSERT INTO users (username,password_hash,full_name,active,created_at) VALUES (\"duxtav_driver\",?,\"DUXT Apache Driver\",1,UTC_TIMESTAMP())")->execute([$hash]);
$pdo->exec("INSERT IGNORE INTO user_roles (user_id, role_id) SELECT (SELECT user_id FROM users WHERE username=\"duxtav_driver\"), role_id FROM roles WHERE code=\"DRIVER\"");
'
rm -f "$REPO_ROOT/api/app/config/config.php"
...
return [
    ...
    'DB_HOST' => 'unused-socket-mode', 'DB_SOCKET' => '$SOCK',
    'DB_NAME' => '$DB_NAME', 'EXPECTED_DB_NAME' => '$DB_NAME',
    'DB_USER' => 'duxtav_runtime_user', 'DB_PASS' => '$RUNTIME_USER_PASS',
    'SESSION_SECURE' => false,
];
PHPCONFIG
...
cat > /etc/apache2/conf-available/duxt-listen.conf <<CONF
Listen $HTTP_PORT
CONF
a2enconf -q duxt-listen
APACHE_CONF_ENABLED=1
cat > /etc/apache2/sites-available/duxt-validate.conf <<CONF
...
curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"duxtav_admin\",\"password\":\"$ADMIN_PASS\"}" > /dev/null
curl -s -c "$DRIVER_JAR" -b "$DRIVER_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"duxtav_driver","password":"ApacheDriverPass123"}' > /dev/null
