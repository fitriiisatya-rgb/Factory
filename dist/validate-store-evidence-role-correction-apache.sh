#!/usr/bin/env bash
# Validates dist/amor-factory-store-evidence-role-correction.zip
...
# from dist/validate-receipt-evidence-apache.sh — this is a FILES-ONLY
# correction (no new migration) on top of that already-deployed patch, so
# it keeps every one of that script's checks (api/app/ deny-all,
# api/assets/ 200s, every prior patch's own section) AND replaces the old
# "Admin can upload evidence" checks with the corrected role-boundary
# ones: THE critical property of THIS specific patch is that Admin can no
# longer upload/replace evidence at all — POST /api/admin/receipts/{id}/
# evidence must 404 through the real front controller (route removed,
# not just hidden), the Admin detail page must never render an upload
# control, and a legacy discrepancy receipt with no store evidence must
# stay PERMANENTLY un-verifiable. Admin can still VIEW store-uploaded
# evidence via GET /api/admin/receipts/evidence/{id} — that part is
# unchanged. This script:
...
DB_NAME="amor_factory_storerole_apachevalidate"
ADMIN_PASS="ApacheStoreRoleAdmin#$(date +%s)"
MIGRATION_USER_PASS="ApacheMigPassSTOREROLE_123"
RUNTIME_USER_PASS="ApacheRunPassSTOREROLE_123"
HTTP_PORT=8196
FPM_STARTED_BY_US=0
...
cleanup() {
  ...
  if [ "$APACHE_SITE_ENABLED" = "1" ]; then a2dissite -q storerole-validate >/dev/null 2>&1 || true; fi
  if [ "$APACHE_CONF_ENABLED" = "1" ]; then a2disconf -q storerole-listen >/dev/null 2>&1 || true; fi
  rm -f /etc/apache2/sites-available/storerole-validate.conf /etc/apache2/sites-enabled/storerole-validate.conf
  rm -f /etc/apache2/conf-available/storerole-listen.conf /etc/apache2/conf-enabled/storerole-listen.conf
...
CREATE USER 'storeroleav_migration_user'@'localhost' IDENTIFIED BY '$MIGRATION_USER_PASS';
GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'storeroleav_migration_user'@'localhost';
CREATE USER 'storeroleav_runtime_user'@'localhost' IDENTIFIED BY '$RUNTIME_USER_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON ${DB_NAME}.* TO 'storeroleav_runtime_user'@'localhost';
FLUSH PRIVILEGES;
"
...
return [
    ...
    'DB_USER' => 'storeroleav_migration_user', 'DB_PASS' => '$MIGRATION_USER_PASS',
...
ADMIN_PASSWORD="$ADMIN_PASS" php "$REPO_ROOT/api/bin/create_admin.php" storeroleav_admin "StoreRole Apache Validate Admin" || exit 1
...
$pdo->prepare("INSERT INTO users (username,password_hash,full_name,active,created_at) VALUES (\"storeroleav_driver\",?,\"StoreRole Apache Driver\",1,UTC_TIMESTAMP())")->execute([$hash]);
$pdo->exec("INSERT IGNORE INTO user_roles (user_id, role_id) SELECT (SELECT user_id FROM users WHERE username=\"storeroleav_driver\"), role_id FROM roles WHERE code=\"DRIVER\"");
'
rm -f "$REPO_ROOT/api/app/config/config.php"
...
return [
    ...
    'DB_HOST' => 'unused-socket-mode', 'DB_SOCKET' => '$SOCK',
    'DB_NAME' => '$DB_NAME', 'EXPECTED_DB_NAME' => '$DB_NAME',
    'DB_USER' => 'storeroleav_runtime_user', 'DB_PASS' => '$RUNTIME_USER_PASS',
    'SESSION_SECURE' => false,
];
PHPCONFIG
...
# Reuses the storeroleav_driver account already created in step 3/10 (real
...
Config::load();
$pdo = Database::pdo();
$driverId = (int) $pdo->query("SELECT user_id FROM users WHERE username=\"storeroleav_driver\"")->fetchColumn();

$prod = new ProductionService($pdo);
$run = $prod->createDraft("'"$TANGGAL"'", '"$DIVISION_ID"', $driverId);
$run = $prod->patchDraft((int) $run["productionRunId"], (int) $run["version"], [["productId" => '"$PRODUCT_ID"', "actualQty" => 5.0]], false, $driverId, "storeroleav-prod-patch");
$prod->submit((int) $run["productionRunId"], (int) $run["version"], $driverId, "storeroleav-prod-submit");

$fg = new FgService($pdo);
$batch = $fg->createDraft("'"$TANGGAL"'", '"$FACTORY_ID"', $driverId);
$batch = $fg->patchDraft((int) $batch["fgBatchId"], (int) $batch["version"], [["productId" => '"$PRODUCT_ID"', "fgVerified" => 5.0, "packed" => 5.0]], false, $driverId, "storeroleav-fg-patch");
$fg->submit((int) $batch["fgBatchId"], (int) $batch["version"], $driverId, "storeroleav-fg-submit");
...
$claim = $dispatch->claim([["doItemId" => $doItemId, "qty" => 5.0]], $driverId, "storeroleav-claim");
...
$result = $departure->confirmDeparture($driverId, '"$DO_ID"', $doVersion, "MAIN", [["claimId" => $claimId, "actualQty" => 5.0]], "storeroleav-depart");
...
cat > /etc/apache2/conf-available/storerole-listen.conf <<CONF
Listen $HTTP_PORT
CONF
a2enconf -q storerole-listen
APACHE_CONF_ENABLED=1
cat > /etc/apache2/sites-available/storerole-validate.conf <<CONF
...
curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"storeroleav_admin\",\"password\":\"$ADMIN_PASS\"}" > /dev/null
curl -s -c "$DRIVER_JAR" -b "$DRIVER_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"storeroleav_driver","password":"ApacheDriverPass123"}' > /dev/null
...
BLOCKED_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/api/receive/$TOKEN/shipments/$SHIPMENT_ID/confirm" \
  -H "Idempotency-Key: storerole-block-$(date +%s)" \
...
CONFIRM_JSON=$(curl -s -X POST "$BASE/api/receive/$TOKEN/shipments/$SHIPMENT_ID/confirm" \
  -H "Idempotency-Key: storerole-confirm-$(date +%s)" \
  -F "receiverName=Toko Apache Test" \
  -F "items=[{\"shipmentItemId\":$SII,\"receivedGood\":4,\"reject\":1,\"shortage\":0}]" \
...
ADMIN_UPLOAD_ATTEMPT_CODE=$(curl -s -o /dev/null -w '%{http_code}' -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/admin/receipts/$RECEIPT_ID/evidence" \
  ...
  -H "Idempotency-Key: storerole-admin-upload-attempt-$(date +%s)" \
  -F "evidence[]=@$EVIDENCE_PNG;type=image/png")
...
VERIFY_JSON=$(curl -s -X POST "$BASE/api/admin/receipts/$RECEIPT_ID/verify" -c "$ADMIN_JAR" -b "$ADMIN_JAR" \
  -H "X-CSRF-Token: $(curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" "$BASE/api/auth/me" | php -r '$j=json_decode(stream_get_contents(STDIN),true); echo $j["data"]["csrfToken"] ?? "";')" \
  -H "Idempotency-Key: storerole-verify-$(date +%s)" -H 'Content-Type: application/json' -d '{}')
