#!/usr/bin/env bash
# Validates dist/amor-factory-shipment-surat-jalan-print-patch.zip against
# a REAL Apache + PHP-FPM server (not `php -S`, which silently ignores
# .htaccess entirely — see SESSION-HANDOFF.md gotcha #1). Adapted from
# dist/validate-driver-ux-tracing-apache.sh for the Driver Route
# Navigation / History / Logout hotfix specifically — also confirms the
# new cache-busting DRIVER_ASSET_VERSION query string on driver.css/
# driver.js/app.js actually resolves under real Apache (the mitigation
# for a stale/skipped-extract deployment being the most likely real-cPanel
# explanation for the History/Logout symptoms — see the final report).
# This script:
...
DB_NAME="amor_factory_sjprint_apachevalidate"
ADMIN_PASS="ApacheSjPrintAdmin#$(date +%s)"
MIGRATION_USER_PASS="ApacheMigPassSJPRINT_123"
RUNTIME_USER_PASS="ApacheRunPassSJPRINT_123"
HTTP_PORT=8196
FPM_STARTED_BY_US=0
...
cleanup() {
  ...
  if [ "$APACHE_SITE_ENABLED" = "1" ]; then a2dissite -q sjprint-validate >/dev/null 2>&1 || true; fi
  if [ "$APACHE_CONF_ENABLED" = "1" ]; then a2disconf -q sjprint-listen >/dev/null 2>&1 || true; fi
  rm -f /etc/apache2/sites-available/sjprint-validate.conf /etc/apache2/sites-enabled/sjprint-validate.conf
  rm -f /etc/apache2/conf-available/sjprint-listen.conf /etc/apache2/conf-enabled/sjprint-listen.conf
...
CREATE USER 'sjprintav_migration_user'@'localhost' IDENTIFIED BY '$MIGRATION_USER_PASS';
GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'sjprintav_migration_user'@'localhost';
CREATE USER 'sjprintav_runtime_user'@'localhost' IDENTIFIED BY '$RUNTIME_USER_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON ${DB_NAME}.* TO 'sjprintav_runtime_user'@'localhost';
FLUSH PRIVILEGES;
"
...
return [
    ...
    'DB_USER' => 'sjprintav_migration_user', 'DB_PASS' => '$MIGRATION_USER_PASS',
...
ADMIN_PASSWORD="$ADMIN_PASS" php "$REPO_ROOT/api/bin/create_admin.php" sjprintav_admin "SJPRINT Apache Validate Admin" || exit 1
...
$pdo->prepare("INSERT INTO users (username,password_hash,full_name,active,created_at) VALUES (\"sjprintav_driver\",?,\"SJPRINT Apache Driver\",1,UTC_TIMESTAMP())")->execute([$hash]);
$pdo->exec("INSERT IGNORE INTO user_roles (user_id, role_id) SELECT (SELECT user_id FROM users WHERE username=\"sjprintav_driver\"), role_id FROM roles WHERE code=\"DRIVER\"");
'
rm -f "$REPO_ROOT/api/app/config/config.php"
...
return [
    ...
    'DB_HOST' => 'unused-socket-mode', 'DB_SOCKET' => '$SOCK',
    'DB_NAME' => '$DB_NAME', 'EXPECTED_DB_NAME' => '$DB_NAME',
    'DB_USER' => 'sjprintav_runtime_user', 'DB_PASS' => '$RUNTIME_USER_PASS',
    'SESSION_SECURE' => false,
];
PHPCONFIG
...
# Reuses the sjprintav_driver account already created in step 3/10 (real
...
Config::load();
$pdo = Database::pdo();
$driverId = (int) $pdo->query("SELECT user_id FROM users WHERE username=\"sjprintav_driver\"")->fetchColumn();

$prod = new ProductionService($pdo);
$run = $prod->createDraft("'"$TANGGAL"'", '"$DIVISION_ID"', $driverId);
$run = $prod->patchDraft((int) $run["productionRunId"], (int) $run["version"], [["productId" => '"$PRODUCT_ID"', "actualQty" => 5.0]], false, $driverId, "sjav-prod-patch");
$prod->submit((int) $run["productionRunId"], (int) $run["version"], $driverId, "sjav-prod-submit");

$fg = new FgService($pdo);
$batch = $fg->createDraft("'"$TANGGAL"'", '"$FACTORY_ID"', $driverId);
$batch = $fg->patchDraft((int) $batch["fgBatchId"], (int) $batch["version"], [["productId" => '"$PRODUCT_ID"', "fgVerified" => 5.0, "packed" => 5.0]], false, $driverId, "sjav-fg-patch");
$fg->submit((int) $batch["fgBatchId"], (int) $batch["version"], $driverId, "sjav-fg-submit");
...
$claim = $dispatch->claim([["doItemId" => $doItemId, "qty" => 5.0]], $driverId, "sjav-claim");
...
$result = $departure->confirmDeparture($driverId, '"$DO_ID"', $doVersion, "MAIN", [["claimId" => $claimId, "actualQty" => 5.0]], "sjav-depart");
...
cat > /etc/apache2/conf-available/sjprint-listen.conf <<CONF
Listen $HTTP_PORT
CONF
a2enconf -q sjprint-listen
APACHE_CONF_ENABLED=1
cat > /etc/apache2/sites-available/sjprint-validate.conf <<CONF
...
curl -s -c "$ADMIN_JAR" -b "$ADMIN_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"sjprintav_admin\",\"password\":\"$ADMIN_PASS\"}" > /dev/null
curl -s -c "$DRIVER_JAR" -b "$DRIVER_JAR" -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"sjprintav_driver","password":"ApacheDriverPass123"}' > /dev/null
